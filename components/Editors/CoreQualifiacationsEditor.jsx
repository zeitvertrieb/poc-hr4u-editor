...
export default function CoreQualificationsEditor({ data, onChange }) {
  ...
  const handleSave = () => {
    if (editIndex === newEntryIndex && newEntryIndex !== null) {
      const isStillEmpty = Object.keys(newEntryDefaults).every(
        (key) => tempEntry[key] === newEntryDefaults[key]
      );
      if (isStillEmpty) {
        ...
        {data.length > 0 ? (
                    ...
                    {isThisRowEditing ? (
                      <div className="flex flex-col gap-2">
                        <input
                          id={`points-${index}`}
                          type="text"
                          value={tempEntry?.points || ''}
                          onChange={(e) =>
                            handleTempEntryChange('points', e.target.value)
                          }
                          className="mt-1 w-full p-1 bg-surface-rise border-b border-border focus:border-interactive-active focus:outline-none text-right"
                          placeholder="z.B. 1.100"
                        />{' '}
                        <p className="text-xs">(1 Jahr = 220 PT)</p>{' '}
                      </div>
                    ) : (
                  ...
                  <div className="flex-1 text-right">
                    ...
                    {isThisRowEditing ? (
                      <input
                        id={`years-${index}`}
                        type="text"
                        value={tempEntry?.years || ''}
                        onChange={(e) =>
                          handleTempEntryChange('years', e.target.value)
                        }
                        className="mt-1 w-full p-1 bg-surface-rise border-b border-border focus:border-interactive-active focus:outline-none text-right"
                        placeholder="z.B. 5"
                      />
                  ...
                  <div className="flex-1">
                    ...
                    {isThisRowEditing ? (
                      <input
                        id={`level-${index}`}
                        type="text"
                        value={tempEntry?.level || ''}
                        onChange={(e) =>
                          handleTempEntryChange('level', e.target.value)
                        }
                        className="mt-1 w-full p-1 bg-surface-rise border-b border-border focus:border-interactive-active focus:outline-none"
                        placeholder="z.B. Ausgezeichnet"
                      />
                    ) : (
          ...
          <div className="text-center py-12 px-6 bg-surface-rise border border-border">
            <h3 className="text-lg font-semibold text-primary">
              Keine Kernqualifikationen vorhanden
            </h3>
            <p className="mt-2 text-sm text-secondary">
              Fügen Sie Ihre erste Qualifikation hinzu, um zu beginnen.
            </p>
            <button
              onClick={handleAddNew}
